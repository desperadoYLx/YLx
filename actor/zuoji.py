...
from dialog.zuoji_dialog import ZUOJIDialog
from dialog.zzh_dialog import ZZHDialog


class ZUOJI(pygame.sprite.Sprite):
    ...
    def __init__(self, pos_x: int, pos_y: int):
        ...
        self.dialog = ZUOJIDialog()
        self.stop = False
        self.action = Action('ZUOJI', '0000', 4, True)
    ...
    def draw(self, surface, x: int, y: int):
        ...
        if self.stop:
            surface.blit(self.dialog.surface, (20, 0))
        header = pygame.transform.scale(image, (200, 130))
        if x < 0:
            x = 0
        if y < 0:
            y = 0
