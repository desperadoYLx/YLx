...
from actor.monster2_battle import BattleMonster2, Monster2BattleStatus
...
class BattleDialog2:
    ...
    def process(self, key_down: bool, pressed_key: int):
        ...
        if self.xiao.status == XIAOBattleStatus.EscapeOver:
            ...
            return
        # 死亡结束
        if self.monster.status == Monster1BattleStatus.DieOver and self.monster2.status == Monster2BattleStatus.DieOver:
        ...
        if self.monster2.status == Monster2BattleStatus.Die:
            self.monster2.action_over()
            if self.monster2.status == Monster2BattleStatus.DieOver:
        ...
        if self.status == BattleStatus.xiao_dialog:
            ...
            if key_down and pressed_key == K_y:
                ...
                self.monster2.set_status(Monster2BattleStatus.Station)
        ...
        if self.status == BattleStatus.xiao_fight:
            if self.xiao.action_over():  # 打斗是一个过程，必须打完
                ...
                self.monster2.set_status(Monster2BattleStatus.Fight)
                # 后减生命值
                if self.monster.status != Monster1BattleStatus.DieOver:
        ...
        if self.status == BattleStatus.Enemy_fight:
            if self.monster.action_over() and self.monster2.action_over():  # 打斗是一个过程，必须打完
