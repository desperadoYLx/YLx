...
from actor.boss_battle import BattleBOSS, BOSSBattleStatus
from actor.diren1_battle import BattleDIREN1, DIREN1BattleStatus
from actor.diren2_battle import BattleDIREN2, DIREN2BattleStatus
...
from dialog.xiao7_dialog import XIAO7Dialog
...
class BattleDialogX:
    ...
    def __init__(self, xiao_hp: int):
        ...
        self.xiao = BattleXIAO(xiao_hp)  # 孙悟空
        self.monster = BattleDIREN1()
        self.monster2 = BattleDIREN2()  # 牛魔王
        self.boss = BattleBOSS()
        # 背景相关初始化
        dialog_file = os.path.join('resource', 'img', 'dialog', '0map30001.png')
    ...
    def process(self, key_down: bool, pressed_key: int):
        ...
        if self.xiao.status == XIAOBattleStatus.EscapeOver:
            ...
            if key_down and pressed_key == K_ESCAPE:
                xsound_path = os.path.join("resource", "music", "zhongmu.mp3")
                pygame.mixer.music.load(xsound_path)
                pygame.mixer.music.play(-1)
                self.over_show = True
                self.over_OVER = False
            return
        # 牛魔王死亡结束
        if self.monster.status == DIREN1BattleStatus.DieOver and\
                self.monster2.status == DIREN2BattleStatus.DieOver and self.boss.status == BOSSBattleStatus.DieOver:
        ...
        # 牛魔王死亡过程
        if self.monster.status == DIREN1BattleStatus.Die:
            self.monster.action_over()
            return

        if self.monster2.status == DIREN2BattleStatus.Die:
            self.monster2.action_over()
            return

        if self.boss.status == BOSSBattleStatus.Die:
            self.boss.action_over()
            if self.boss.status == BOSSBattleStatus.DieOver:
        ...
        if self.status == BattleStatus.xiao_dialog:
            ...
            if key_down and pressed_key == K_y:
                ...
                if self.monster.status != DIREN1BattleStatus.DieOver:
                    self.monster.set_status(DIREN1BattleStatus.Station)
                if self.monster2.status != DIREN2BattleStatus.DieOver:
                    self.monster2.set_status(DIREN2BattleStatus.Station)
                self.boss.set_status(BOSSBattleStatus.Station)
        ...
        if self.status == BattleStatus.xiao_fight:
            if self.xiao.action_over():  # 打斗是一个过程，必须打完
                ...
                if self.monster.status != DIREN1BattleStatus.DieOver:
                    self.monster.set_status(DIREN1BattleStatus.Fight)
                if self.monster2.status != DIREN2BattleStatus.DieOver:
                    self.monster2.set_status(DIREN2BattleStatus.Fight)
                self.boss.set_status(BOSSBattleStatus.Fight)
                # 后减生命值
                if self.monster.status != DIREN1BattleStatus.DieOver:
                    self.monster.attack_hp(-10)
                if self.monster.status == DIREN1BattleStatus.DieOver and\
                        self.monster2.status != DIREN2BattleStatus.DieOver:
                    self.monster2.attack_hp(-10)
                if self.monster2.status == DIREN2BattleStatus.DieOver :
                    self.boss.attack_hp(-10)
        ...
        if self.status == BattleStatus.Enemy_fight:
            if self.monster.action_over() and self.monster2.action_over() and self.boss.action_over():  # 打斗是一个过程，必须打完
    ...
    def draw(self, surface):
        ...
        if self.monster.status == DIREN1BattleStatus.DieOver and self.monster2.status == DIREN2BattleStatus.DieOver and\
                self.boss.status == DIREN2BattleStatus.DieOver:
            dialog.blit(XIAO7Dialog().surface, (0, 0))
            blit_text(dialog, self.text2, (375, 250), self.font)

        self.xiao.draw(dialog)
        self.monster.draw(dialog)
        self.monster2.draw(dialog)
        self.boss.draw(dialog)
